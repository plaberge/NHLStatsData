CREATE PROCEDURE [dbo].[spGetSchedulewithLatestGameEvents]
	@scheduleDate datetime 
AS
BEGIN

	SELECT ge.Id 
	INTO #tblTempGameEvents
	FROM tblSchedule s, tblGame g, tblGameEvent ge
	WHERE
		s.scheduleDate = @scheduleDate AND
		s.Id = g.scheduleID AND
		ge.correspondingGameId = g.gameId AND
		ge.eventTimeStamp in (SELECT MAX(eventTimeStamp) FROM tblGameEvent where correspondingGameId = g.gameId)
    ...
    SELECT 
		...
		WHERE 
			ge.Id IN (SELECT Id FROM #tblTempGameEvents) AND
			g.scheduleId = dbo.fnGetScheduleIdFromDate(@scheduleDate) AND
			ge.correspondingGameId = g.gameId AND
